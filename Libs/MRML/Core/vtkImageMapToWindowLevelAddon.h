...
public:
  ...
  static inline double mapScalarToWindow(T inputValue, double rangeMin, double rangeMax, WindowMappingMode mappingMode) noexcept
  {
    ...
    switch (mappingMode)
    {
      case WindowMappingMode::Linear: return std::clamp((inVal - rangeMin) / rangeWidth, 0.0, 1.0);
      case WindowMappingMode::LogCompressLowValues: return std::log10(std::clamp(1.0 + 9.0 * (inVal - rangeMin) / rangeWidth, 1.0, 10.0));
      case WindowMappingMode::LogCompressHighValues: return 1.0 - std::log10(std::clamp(1.0 + 9.0 * (rangeMax - inVal) / rangeWidth, 1.0, 10.0));
    }
    return 0.0;
  }
